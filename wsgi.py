...
def seed_demo_data():
    ...
    election_id = execute_db(
        ...
        ('BCA Student Council Election 2026', 'Annual student council election for BCA department', 'Student Council', 'All Constituencies',
         (now - datetime.timedelta(hours=1)).strftime('%Y-%m-%d %H:%M:%S'), (now + datetime.timedelta(days=7)).strftime('%Y-%m-%d %H:%M:%S'))
    )

    # Candidates
    if election_id:
        candidates_data = [
            (election_id, 'Candidate A', 'Student Alliance', 'Star', 'Experienced student leader focused on academics and campus welfare.'),
            (election_id, 'Candidate B', 'Progress Forum', 'Laptop', 'Dedicated to improving campus infrastructure and student activities.'),
            (election_id, 'Candidate C', 'Independent', 'Handshake', 'Independent candidate committed to transparency and student rights.'),
        ]
    ...
    past_id = execute_db(
        ...
        ('BCA Tech Quiz Competition 2025', 'Annual tech quiz team election', 'Quiz Team', 'All Constituencies',
         (now - datetime.timedelta(days=60)).strftime('%Y-%m-%d %H:%M:%S'), (now - datetime.timedelta(days=53)).strftime('%Y-%m-%d %H:%M:%S'))
    )

    if past_id:
        past_candidates = [
            (past_id, 'Alpha Squad', 'Alpha Team', 'A', 'Previous winners of the inter-class competition.'),
            (past_id, 'Beta Warriors', 'Beta Team', 'B', 'Strong competitors in coding challenges.'),
            (past_id, 'Gamma Stars', 'Gamma Team', 'Star', 'New team with innovative ideas.'),
        ]
...
with app.app_context():
    # Auto-create tables and seed data (works for MySQL, PostgreSQL, and SQLite)
    try:
        init_database()
